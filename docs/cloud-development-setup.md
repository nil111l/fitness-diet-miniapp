...
3. 项目的基础平台食材已初始化后，首次读取食谱或生成一日计划会幂等初始化 12 份基础食谱。重复调用不会产生重复数据，已被后台下架或删除的基础食谱不会自动恢复。
4. 可继续通过后台“食材管理”和“食谱管理”维护平台食材、标签、食谱与上下架状态。
5. 食谱保存时云函数会按平台食材每百克营养和克数计算每份营养；食谱下架后公开列表、详情、加入餐次和饮食计划都会拒绝继续使用。
6. 首次读取食谱标签时会初始化“高蛋白、低脂、低碳”三个基础标签，管理员可继续新增或停用。

## Phase 8 训练内容配置

1. 上传并部署 `workout` 云函数，选择“云端安装依赖”。
2. 首次调用后会自动创建 `exercise_actions`、`workout_plans`、`user_workout_plans`、`workout_sessions`，并幂等初始化 9 个基础动作和 6 套免费训练计划。
